...
public class SharedLogic {
    ...
    public static void fnIIDFDeleteOrphans(String tableName, Object perantTble, String targetIid, String pos, Long op_ts, Boolean run_parent_delete, Object opetaion, Long cr_ts, Boolean logicalChildChanged) throws Exception {
        ...
        for (SerializeFabricLu.Table child : table_Object.tableChildren) {//Loop throw all table's childs and delete orphan records
            ...
            ludb().execute(String.format(delete, getLuType().luName, child.tableName, sqlQuery.toString()));//Delete orphan records
            if (rec_deleted) {
                fnIIDFDeleteOrphans(child.tableName, perantTble, targetIid, pos, op_ts, false, ope, cr_ts, false);//Call function again with child table
            }
        }
    }
    ...
    public static void fnIIDFSyncDelta() throws Exception {
        final String migrate = "MIGRATE %s FROM %s USING ('select DISTINCT iid from %s.%s where iid IN (%s)')";
        //User job to run in the background and sync instances from IIDF
        com.k2view.cdbms.finder.BulksIterator it = new com.k2view.cdbms.finder.BulksIterator(getLuType().luName, (iids) -> {
            try {
                String escapingIids = iids.stream().map(s -> "\''" + s + "\''").collect(java.util.stream.Collectors.joining(","));
                fabric().execute(String.format(migrate, getLuType().luName, DB_CASS_NAME, IidFinderUtils.getKeyspace(), IidFinderUtils.getDeltaTableName(IidFinderUtils.getAlias(getLuType().luName)), escapingIids));
            } catch (Exception e) {
    ...
    public static Map<String, Set<String>> fnIIDFGetParentTable(String lut_name) {
        //Function to get table parent
        Map<String, Set<String>> prntTable = new HashMap<String, Set<String>>();
    ...
    public static void fnIIDFSendRecBack2Kafka(String table_name, Object column_list, Object val_list, String targetIid, String pos, Long op_ts, String reason, Long cr_ts) throws Exception {
        ...
        String[] pkCuls = (String[]) fnIIDFGetTablePK(table_name, getLuType().luName);
        String prefix = "";
    ...
    public static void fnIIDFPopRecentUpdates(Object[] valueArrays, Long opTimeStamp, String iid, String sql, Long maxUpdated, Long optCurrentTimeStamp, Long idfinderHandleTime) throws Exception {
        ...
        ludb().execute("insert or replace into " + getLuType().luName + ".IIDF_RECENT_UPDATES (IID,SQL,VAL_ARRAY,OP_TIMESTAMP,CURRENT_TIMESTAMP,OP_CURRENT_TIMESTAMP, IDFINDER_HANDLE_TIME) values (?,?,?,?,?,?,?)", new Object[]{iid, sql, strArray.toString(), opTimeStamp, maxUpdated, optCurrentTimeStamp, idfinderHandleTime});
    }
    ...
    public static void fnIIDFPopTable(String IID) throws Exception {
        ...
        for (Map.Entry<String, List<Map<String, Object>>> keysToRemove : idfTablesDeleteList.entrySet()) {
            for (Map<String, Object> DCKeys : keysToRemove.getValue()) {
                ...
                if(!"false".equals(((TableObject) getLuType().ludbObjects.get(keysToRemove.getKey())).getEnabledTablePopulationObjects().get(0).iidFinderProp.isStoreData))addDeleteFinder(getLuType().luName, getInstanceID(), keysToRemove.getKey(), keys);
            }
        }
	...
	public static void fnIIDFConsMsgs(String IID) throws Exception {
		...
		final String IIDFQueueDelete = "DELETE FROM %s.IIDF_QUEUE";
		final String insertStmt = "insert into %s.IIDF_QUEUE (DATA_CHANGE , iid , opt_timeStamp , current_timeStamp, POS ) values (?,?,?,?,?)";
		final String IIDF = "select MAX_DC_UPDATE from %s.IIDF ";
		final String IIDFRecentUpdatesSDelete = "delete from %s.IIDF_RECENT_UPDATES where \"current_timeStamp\" < ?";
		...
		if (maxDCUpdate != null) maxHandleTime = Long.parseLong(maxDCUpdate.toString());
		...
		try (Stream<DataChange> dataChangesFromDelta = IidFinderApi.getDeltas(getLuType().luName, IID, maxHandleTime)) {//Fetch all DCs newer then maxHandleTime
		    Iterable<DataChange> dataChangesFromDeltaItr = () -> dataChangesFromDelta.iterator();
		    for (DataChange dc : dataChangesFromDeltaItr) {//Loop throw all DCs and insert to IIDF_QUEUE LU Table
		        ...
		        if (STATS_ACTIVE != null && Boolean.parseBoolean(STATS_ACTIVE)) {
		...
		paramsStats.add((Duration.between(start, Instant.now()).toMillis()) - dbExecuteStartTimeTTL);//Time To Fetch Deltas From IDFInder API
		...
		start = Instant.now();
		try (Db.Rows iidfQRS = ludb().fetch(String.format(IIDFQueue, getLuType().luName, filterQueue), params)) {
		    for (Db.Row row : iidfQRS) {
		        ...
		        if (dataChange.getOperation().equals(DataChange.Operation.insert) && dataChange.getOrgOperation().equals(DataChange.Operation.update))
		            fnIIDFForceTableSyncFromSource(dataChange);
		...
		if ("true".equalsIgnoreCase(STATS_ACTIVE)) fnIIDFUpdateStats(statsMap);
		...
		paramsStats.add(crossInstanceList.size()); //Total cross DCs count
		...
		ludb().execute(String.format(IIDFRecentUpdatesSDelete, getLuType().luName), new Object[]{maxUpdated - time_to_keep * 60 * 60 * 1000 * 1000});
        ...
        if (!translationOutput.isEmpty()) {
            ...
            if (translationOutput.get("function_name") != null) {
                ...
                for (String userMethod : userMethodsLst) {
                    ...
                    Instant startTime = Instant.now();
                    getLuType().invokeFunction(userMethod, null, new Object[]{dataChange, functionContext()});
    ...
    public static void fnIIDFUpdateStats(Object statsMapInp) throws Exception {
        ...
        for (Map.Entry<String, Map<String, Integer>> mapEnt : statsMap.entrySet()) {
            ...
            Db.Row rowData = null;
            try (Db.Rows rows = fabric().fetch("Select TOTAL_UPDATE, TOTAL_INSERT, TOTAL_DELETE, TOTAL_REPLICATE, TOTAL_IIDCHANGE from IIDF_STATISTICS where LU_NAME = ? and IID = ? and TABLE_NAME = ?", new Object[]{getLuType().luName, getInstanceID(), mapEnt.getKey()})) {
                    ...
                    if (getThreadGlobals("childCrossInsts" + childTblName.toUpperCase()) != null) {
                        ...
                        for (String parentLink : fullParentLinksArr) {
                            if (allKeysMap.containsKey(parentLink.toUpperCase())) {
                                ...
                                String prefix = "";
                                String[] pkCuls = (String[]) fnIIDFGetTablePK(tableName, getLuType().luName);
                                for (String pkCul : pkCuls) {
    ...
    public static void fnIIDFExecInsNullPK(Object dc) throws Exception {
        ...
        StringBuilder updateStmt = new StringBuilder().append(" update " + getLuType().luName + "." + dataChange.getTargetTableName() + " set ");
        ...
        Object rs = null;
        try (Db.Rows rs2 = ludb().fetch("select 1 from " + getLuType().luName + "." + dataChange.getTargetTableName() + whereStmt.toString(), whereVals.toArray())) {
            rs = rs2.firstValue();
        }
    ...
    public static Boolean fnIIDFDeleteTableOrphanRecords(String tableName, Object parent_tables, String targetIid, String pos, Long op_ts, Long cr_ts) throws Exception {
        ...
        boolean rec_deleted = fnIIDFGetRecData(tableName, sqlQuery.toString(), targetIid, pos, op_ts, (!Boolean.parseBoolean(IIDF_EXTRACT_FROM_SOURCE) || getTranslationsData("trnIIDFForceReplicateOnDelete").keySet().contains(tableName.toUpperCase())) ? true : false, cr_ts);//Before deleting records send them to kafka
    ...
    public static void fnIIDFInsertIIDStats() throws Exception {
        ...
        class myRunnable implements Runnable {
            ...
            public myRunnable(String iid, List<Object> paramsStats, long replicates_send_back_r, long replicates_send_back_d, long ttlSyncTime, UserCodeDelegate i_ucd, long accountChange, long is_iid_change, boolean iid_skipped) {
                this.iid = iid;
    ...
    public static Boolean fnIIDFExecNonLUTableDelta(Object i_dataChange) throws Exception {
        ...
        if (Boolean.parseBoolean(trnMap.get("ACTIVE"))) {
            ...
            if (Boolean.parseBoolean(trnMap.get("SKIP"))) {
                return true;
            } else {
                return false;
            }
    ...
    public static void fnIIDFCleanIIDFinderCTables(Object i_dc) throws Exception {
        ...
        for (LudbObject childTable : getLuType().ludbObjects.get(LUTableName).childObjects) {
            ...
            if (rubDF && !"false".equals(((TableObject) getLuType().ludbObjects.get(LUTableName)).getEnabledTablePopulationObjects().get(0).iidFinderProp.isStoreData)) addDeleteFinder(getLuType().luName, getInstanceID(), LUTableName, keys);
            ...
            for (String linkedColumn : linkedColumns.split(",")) {
                if (dcAfterValues.containsKey(linkedColumn)) {
                    ...
                    if (!rowColumnValue.equals(afterValue)) {
                        ...
                        rubDF = true;
                        if(!"false".equals(((TableObject) getLuType().ludbObjects.get(LUTableName)).getEnabledTablePopulationObjects().get(0).iidFinderProp.isStoreData))addDeleteFinder(getLuType().luName, getInstanceID(), LUTableName, keys);
                        break;
                    }
                }
            }
    ...
    public static Boolean fnIIDFCheckRExists(Object i_dc) throws Exception {
        ...
        if (ludb().fetch(String.format(select, getLuType().luName, tableName, recKeys.toString()), keyVals.toArray()).firstValue() == null) {
            return false;
        } else {
            return true;
        }
	...
	public static void fnIIDFExecPreSyncUserActivity() throws Exception {
		Map<String, Long> debugTiming = (Map<String, Long>) getThreadGlobals("DEBUG_TIMING_MAP");
		for (Map.Entry<String, Map<String, String>> trnData : getTranslationsData("trnIIDFExecPreSyncUserActivity").entrySet()) {
		    ...
		    if (x == null || "null".equalsIgnoreCase(x) || "".equals(x) || !(Boolean.parseBoolean(y.get("active"))))
		        return;
		    Instant startTime = Instant.now();
		    getLuType().invokeFunction(x, null, new Object[]{functionContext()});
		    debugTiming.put("fnIIDFExecPreSyncUserActivity: " + x, Duration.between(startTime, Instant.now()).toMillis());
		}
	}


    public static void fnIIDFForceTableSyncFromSource(Object i_dc) throws Exception {
        DataChange dataChange = (DataChange) i_dc;
        Map<String, Object> dcTableColumns = dataChange.getValues();
        String tableName = dataChange.getTargetTableName();
        Map<String, String> tableColumns = (Map<String, String>) fnIIDFGetTablesCoInfo(tableName, getLuType().luName);
